<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 leading-tight">Proveedores</h2></x-slot>

    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-between items-center mb-4">
                <h1 class="text-2xl font-bold">Proveedores</h1>
                <a href="{{ route('proveedores.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded">Nuevo</a>
            </div>
                    ...
                    <tbody>
                        @forelse($proveedores as $p)
                            <tr>
                                ...
                                <td class="p-2 border">{{ $p->documento }}</td>
                                <td class="p-2 border">{{ $p->telefono }}</td>
                                <td class="p-2 border">{{ $p->email }}</td>
                                <td class="p-2 border">{{ $p->ciudad }}</td>
                                <td class="p-2 border">
                                    <a href="{{ route('proveedores.edit',$p) }}" class="text-indigo-600">Editar</a>
