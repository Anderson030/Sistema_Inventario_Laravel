<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('envios.store') }}" class="bg-white p-6 rounded shadow space-y-4" id="formEnvio">
                @csrf

                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <x-input-label value="valor envio" />
                    <x-text-input name="valor_envio" type="text" class="md:col-span-2 cop" placeholder="$ 0" required />

                    <x-input-label value="numero bulto" />
                    <x-text-input name="numero_bulto" type="number" min="1" class="md:col-span-2" required />

                    <x-input-label value="valor bulto" />
                    <x-text-input name="valor_bulto" type="text" class="md:col-span-2 cop" placeholder="$ 0" required />

                    <x-input-label value="ganacia total" />
                    <x-text-input name="ganancia_total" type="text" class="md:col-span-2 cop" placeholder="$ 0" required />

                    <x-input-label value="pago contado" />
                    <x-text-input name="pago_contado" type="text" class="md:col-span-2 cop" placeholder="$ 0" />

                    <x-input-label value="pago a plazo" />
                    <x-text-input name="pago_a_plazo" type="text" class="md:col-span-2 cop" placeholder="$ 0" />
    ...
    {{-- Formato COP en inputs .cop --}}
    <script>
    (function(){
        const fmt = new Intl.NumberFormat('es-CO', { style:'currency', currency:'COP', maximumFractionDigits: 0 });

        function unformatCop(str){
            if(!str) return '';
            return (str+'').replace(/[^\d]/g,''); // deja solo dígitos
        }
        ...
        // antes de enviar, deja solo dígitos (el backend ya espera string y normaliza)
        document.getElementById('formEnvio').addEventListener('submit', ()=>{
            document.querySelectorAll('input.cop').forEach(el=>{
                el.value = unformatCop(el.value);
            });
        });
    })();
    </script>
